...
CREATE TABLE Files (
    ...
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
    CONSTRAINT FK_Files_Repositories FOREIGN KEY (RepositoryId) REFERENCES Repositories(Id)
);
...
CREATE TABLE AnalysisScores (
    ...
    CONSTRAINT FK_AnalysisScores_Analyses FOREIGN KEY (AnalysisId) REFERENCES Analyses(Id)
);
...
CREATE TABLE Reports (
    ...
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
    CONSTRAINT FK_Reports_Analyses FOREIGN KEY (AnalysisId) REFERENCES Analyses(Id)
);
...
CREATE TABLE InterviewQuestions (
    ...
    CONSTRAINT FK_InterviewQuestions_Sessions FOREIGN KEY (InterviewSessionId) REFERENCES InterviewSessions(Id)
);
...
CREATE TABLE InterviewResults (
    ...
    CreatedAt DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
    CONSTRAINT FK_InterviewResults_Sessions FOREIGN KEY (InterviewSessionId) REFERENCES InterviewSessions(Id)
);
...
CREATE INDEX IX_Files_RepositoryId_Path ON Files(RepositoryId, Path);
CREATE INDEX IX_Analyses_RepositoryId_StartedAt ON Analyses(RepositoryId, StartedAt DESC);
CREATE INDEX IX_AnalysisScores_AnalysisId ON AnalysisScores(AnalysisId);
CREATE INDEX IX_AuditLogs_ActorUserId_CreatedAt ON AuditLogs(ActorUserId, CreatedAt DESC);
CREATE INDEX IX_Sessions_UserId_ExpiresAt ON Sessions(UserId, ExpiresAt);
